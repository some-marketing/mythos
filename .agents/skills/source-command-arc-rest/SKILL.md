---
name: source-command-arc-rest
description: "Transition the current actor arc to resting with a recorded reason"
---

# /arc-rest

Canonical authority: `instructions/canonical/commands/arc-rest.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
