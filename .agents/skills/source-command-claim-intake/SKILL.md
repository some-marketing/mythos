---
name: source-command-claim-intake
description: "Claim a client-board intake item and advance it into bounded planning"
---

# /claim-intake

Canonical authority: `instructions/canonical/commands/claim-intake.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
