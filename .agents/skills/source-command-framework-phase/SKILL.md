---
name: source-command-framework-phase
description: "Base spec for a single framework phase command"
---

# /framework-phase

Canonical authority: `instructions/canonical/commands/framework-phase.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
