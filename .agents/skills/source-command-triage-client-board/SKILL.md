---
name: source-command-triage-client-board
description: "Triage client-board intake into pickup-ready work, planning work, and clarification work"
---

# /triage-client-board

Canonical authority: `instructions/canonical/commands/triage-client-board.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
