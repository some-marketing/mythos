...
5. **Route closeout to one of three lanes:**
   - **verify-local** — only for low-risk, small, single-surface, repo-local slices with no credential, browser-auth, or staging/production dependency. Record a durable verification artifact under `_dev/reports/analysis/`. If verification escalates or errors, do not treat the slice as cleared — route it onward.
