...
<objective>
Inscribe lore: create a new concept file or concept bundle under `concepts/`. A concept is a durable record of a decision and its reasoning. The policy is flat by default, and a bundle when the work needs accumulated context or cross-model dispatch alongside the concept.
</objective>

<process>
1. Parse arguments for a concept slug (required, kebab-case) and the optional `--bundle` flag.
2. **Flat (default):** create `concepts/<slug>.md` with standard frontmatter (title, identified date, context) and sections for Problem, Decision, Rationale, and Next Steps.
3. **Bundle (`--bundle`):** create `concepts/<slug>/concept.md` with the same frontmatter, a `status.json` (slug, created date, author, stage `draft`, empty dispatches, null promoted_to), and empty `context/` and `dispatch/` directories.
