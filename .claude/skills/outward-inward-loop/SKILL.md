...
Before a comparative trial can graduate to `RUN_ONLY`, a `REVIEW_ONLY` prompt-provenance attester must inspect every normalized candidate prompt against its external source envelope for embedded instructions and untrusted executable behavior. First write a `PromptProvenanceSourceManifest/1.0` with a stable `source_envelope_id`, a non-empty `sources` array containing each `source_id` and `content_sha256`, and `prompt_sources` mapping every runnable prompt id to the exact source ids it derives from. Set `rewrite_performed` to `true` only when a separate human or rewriter actor performs a rewrite; the receipt must then include that actor's id, provider family, and resolved model id, while the attester may not rewrite and attest the same prompt. For the documented no-rewrite path, set `rewrite_performed` to `false` and omit rewriter identity fields. Write a `PromptProvenanceReceipt/1.0` entry for every runnable prompt with `prompt_id`, `prompt_sha256`, `source_envelope_id`, `source_envelope_sha256`, `source_revisions` containing the assigned `source_id` and `content_sha256` pairs, `rewrite_performed`, attester actor id, provider family, resolved model id, and `attestation`, then run `node tools/commands/validate-prompt-provenance-receipt.cjs --receipt <path> --candidate <candidate-or-framework-root> --source-manifest <path>`. The checker compares normalized identities, provider families, and resolved model ids when rewriting occurred, and binds every prompt entry to the exact current prompt set, prompt-to-source assignment, source revisions, and source-manifest identity and digest, so a source refresh or wrong-source attestation invalidates the receipt. It is currently `ADVISORY`, not a blocking runtime gate, because no registered comparative executor invokes it. Comparative execution capability is therefore `ABSENT`: design the trial, stop before prompt-chain execution, and record `execution_blocked` until a registered runner invokes the validator before every run.
...
2. Use at least one neutral common-core case with the same input, outcome contract, tool envelope, context allowance, resource budget, and independent evaluator for every arm. The trial contract must enumerate every required artifact in each candidate's `output_contract_v2` (or compatibility `output_contract`), and a future runner must validate the complete declared artifact set—including required path patterns, schemas, digests, and producer-stage bindings where declared—rather than treating the existence of an output directory or one artifact as completion. Missing, unexpected, stale, schema-invalid, or incorrectly produced artifacts make that arm `invalid_control`. Record every dimension that still differs, including candidate rank and maturity.
...
6. Plan a future `RUN_ONLY` producer for prompt-chain execution and a `REVIEW_ONLY` evaluator whose actor id and model-provider family are both distinct from the producer. Blind arm identity during evaluation when the artifact format permits it. Neither the importer, candidate, incumbent, nor producer validates its own trial. Do not dispatch the producer while comparative execution capability remains `ABSENT`.
...
- staged candidate replay evidence — readiness receipt plus a separate comparative-trial design and `execution_blocked` receipt when `--comparative-test` is requested.
