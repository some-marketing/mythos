...
- Combined projector, lifecycle, and alias regressions: 33 passed, 0 failed.
...
- 189 evidence receipts emitted; bundled-resource packages carry deterministic path/hash/mode manifests and complete-package hashes. Primary sources and resources must be regular non-symlink files contained by their declared source roots before reading. Executable modes are preserved on newly written resources and checked thereafter. No collisions and no unclassified `UNKNOWN` projection was applied.
