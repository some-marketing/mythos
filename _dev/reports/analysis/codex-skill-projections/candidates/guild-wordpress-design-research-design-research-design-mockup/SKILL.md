---
name: guild-wordpress-design-research-design-research-design-mockup
description: "Creates and maintains HTML design mockups informed by competitive audit data. Extracts live site computed styles via Playwright MCP for 1:1 chrome wrapping, generates standalone preview files, maintains a living design spec document, and audits all artifacts for consistency. Use for inventory page redesigns, VDP layouts, or any client page where a visual mockup is needed before build."
---

Framework lineage: `frameworks/wordpress/design-research`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/design-research/.claude/skills/design-research/design-mockup/SKILL.md`.
