---
name: guild-wordpress-design-research-design-research-mockup-brief
description: "Bridges the gap between competitive site audit and design mockup creation. Reads audit evidence (FEATURE_MATRIX.md, site analyses), guides the user through design direction decisions, extracts brand tokens from the live site, and produces a structured mockup_brief.json + initial spec document skeleton. This brief becomes the input for /framework:mockup, replacing ad-hoc conversational decisions with a documented, auditable handoff artifact."
---

Framework lineage: `frameworks/wordpress/design-research`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/design-research/.claude/skills/design-research/mockup-brief/SKILL.md`.
