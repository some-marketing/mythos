---
name: guild-wordpress-qa-qa-append-to-dev-bundle
description: "Appends one or more testcase runs to an existing payload-reporting developer handoff bundle, updating bundle indexes (INDEX.md, INDEX.json) and the For_Recipient.md summary. Used when a bundle already exists and new runs need to be added without creating a fresh bundle."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/append-to-dev-bundle/SKILL.md`.
