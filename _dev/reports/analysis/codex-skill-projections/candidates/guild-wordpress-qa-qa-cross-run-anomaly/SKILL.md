---
name: guild-wordpress-qa-qa-cross-run-anomaly
description: "Builds a cross-run index of recurring anomalies and regressions across many runsets, with stable classification buckets and evidence pointers. Used to identify trends, regression windows, and systemic issues across the test suite."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/cross-run-anomaly/SKILL.md`.
