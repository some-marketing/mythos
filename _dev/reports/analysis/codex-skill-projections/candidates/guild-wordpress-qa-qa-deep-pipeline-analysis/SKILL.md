---
name: guild-wordpress-qa-qa-deep-pipeline-analysis
description: "Traces expected values through the full pipeline (identity -) WPForms -) CRM) and produces a field-by-field truth table with actionable synthesis report. Used when the UI run passes but backend data is wrong or missing, or when mapping contract drift is suspected."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/deep-pipeline-analysis/SKILL.md`.
