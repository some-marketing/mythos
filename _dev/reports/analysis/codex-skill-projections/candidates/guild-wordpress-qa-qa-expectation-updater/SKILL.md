---
name: guild-wordpress-qa-qa-expectation-updater
description: "Updates testcase expectation files (EXPECTED_OUTCOMES.md, expected_payload.json) based on a dev changelog. Parses behavioral and data format changes, maps them to expectation fields, presents proposed updates for user confirmation, and applies approved changes. Use after changelog-capture when testcase expectations need to reflect intentional behavioral changes."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/expectation-updater/SKILL.md`.
