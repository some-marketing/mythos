---
name: guild-wordpress-qa-qa-implement-fixes
description: "Applies the smallest set of repo changes needed to fix failing testcase environments, using run artifacts and walkthrough findings as evidence. Operates in PATCH_ALLOWED mode with traceability from every change back to its evidence source. Use after triage or walkthrough has identified actionable root causes."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/implement-fixes/SKILL.md`.
