---
name: guild-wordpress-qa-qa-iterate-until-pass
description: "Coordinates the end-to-end iteration loop for driving a failing testcase to stable PASS across all required environments. Orchestrates run, triage, stakeholder gate, walkthrough, fix, rerun, and report steps, delegating to canonical prompts 04/05/07/08/03/10 as needed. Use when a testcase is failing or flaky and needs a repeatable loop to reach stability."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/iterate-until-pass/SKILL.md`.
