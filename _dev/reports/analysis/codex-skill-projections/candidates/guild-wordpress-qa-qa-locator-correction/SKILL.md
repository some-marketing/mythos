---
name: guild-wordpress-qa-qa-locator-correction
description: "Validates and corrects a locator map through live browser walkthrough. Navigates the form page by page, documents actual DOM behavior, identifies selector drift, conditional fields, popup interstitials, and non-standard widgets, then produces a findings report with corrections. Use after initial scaffold, when tests fail due to selector issues, or for periodic maintenance."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/locator-correction/SKILL.md`.
