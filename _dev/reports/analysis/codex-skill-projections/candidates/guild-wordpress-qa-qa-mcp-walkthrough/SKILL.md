---
name: guild-wordpress-qa-qa-mcp-walkthrough
description: "Troubleshoots automation flows via live browser walkthrough using Playwright MCP, documenting findings without applying fixes. Operates in FINDINGS_ONLY mode to observe selector resolution, page transitions, wait conditions, and DOM state. Use when a testcase environment is failing and you need live DOM evidence before implementing changes."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/mcp-walkthrough/SKILL.md`.
