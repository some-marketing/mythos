---
name: guild-wordpress-qa-qa-parallel-run
description: "Executes a testcase across A/B/C environments in parallel using subagents, compiles per-env reports and a manager summary. Operates in strict RUN_ONLY mode with no fixes, no reruns, and no code modifications. Allocates a runset, spawns one subagent per environment, triages results, and produces a manager report. Use when ready to execute a validated testcase."
---

Framework lineage: `frameworks/wordpress/qa`. Read its `manifest.json` and `guardrails.md` before execution. Source helper: `frameworks/wordpress/qa/.claude/skills/qa/parallel-run/SKILL.md`.
