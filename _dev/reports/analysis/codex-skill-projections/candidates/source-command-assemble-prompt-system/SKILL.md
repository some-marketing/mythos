---
name: source-command-assemble-prompt-system
description: "Reconcile prompt packs, run prompts, manifest, and the master run order"
---

# /assemble-prompt-system

Canonical authority: `instructions/canonical/commands/assemble-prompt-system.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
