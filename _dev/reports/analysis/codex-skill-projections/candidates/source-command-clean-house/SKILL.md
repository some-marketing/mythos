---
name: source-command-clean-house
description: "Group dirty files by workstream, propose scoped commits, clean stale artifacts"
---

# /clean-house

Canonical authority: `instructions/canonical/commands/clean-house.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
