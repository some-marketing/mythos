---
name: source-command-context-check
description: "Read current session's Tier 0 contextual hints — surfaces relevant memories, live signals, and ledger entries"
---

# /context-check

Canonical authority: `instructions/canonical/commands/context-check.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
