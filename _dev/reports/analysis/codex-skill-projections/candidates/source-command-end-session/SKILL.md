---
name: source-command-end-session
description: "Programmatically inventory end-session closeout evidence and emit an EndSessionCloseout/1.0 readiness index"
---

# /end-session

Canonical authority: `instructions/canonical/commands/end-session.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
