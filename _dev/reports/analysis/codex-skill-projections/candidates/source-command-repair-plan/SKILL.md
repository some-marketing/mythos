---
name: source-command-repair-plan
description: "Repair defective plan artifacts via atomic paired JSON+MD authority-field mutation with full provenance"
---

# /repair-plan

Canonical authority: `instructions/canonical/commands/repair-plan.yaml`. Read that file at execution time; this projection never copies or overrides its behavioral body.

Capability tier: **ADVISORY**.

Read the canonical command at execution time and carry out its workflow with Codex capabilities. This projection is ADVISORY; availability of this skill is not a blocking runtime mechanism.
