...
the VM is Off: `psrunfile.sh load-courier.ps1 -ExpectedSha256 <sha256 printed by
build-export.sh>`, then refresh the seed and run the approved provisioning
boot with the same hash: `psrunfile.sh first-boot.ps1 -ExpectedSha256
<sha256>`. Both scripts fail closed if the payload they find does not match
`-ExpectedSha256` — the loader against the newest archive in staging, and
first-boot against the archive actually present on the courier immediately
before boot — so a stale or missing transfer is caught before it burns a
20-minute boot window instead of being discovered after. Both scripts also
validate that `-ExpectedSha256` is shaped like a sha256 digest (64 hex
characters) immediately on entry, before touching the VM or courier, so a
malformed argument fails with a specific message rather than a confusing
downstream hash mismatch. The loader also retains the prior sibling-`.sha256`
self-consistency check as a secondary integrity check.
...
Publication is manifest-last-as-completion-marker: the archive is renamed into
place first, then the `.sha256`, then the `.MANIFEST.txt` last. A manifest's
presence marks a complete triple; anything selecting a payload should key on
the manifest, not on the newest archive by glob, to avoid picking up a triple
still mid-write.
