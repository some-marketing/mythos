...
// tools/ant-hive-world/run-live.js — S3 first attended run driver.
//
...
// the sim IS the training ground (operator, 2026-07-16). Superseded
// llm-decide.js/Ollama entirely per the operator's "no no we need an
// untrained model" correction; see
// _dev/reports/analysis/run-debrief__ant-hive-world-untrained-network-buildcost.md.
//
// Operator present per G-ATTENDED-FIRST-RUN; monitoring is log-based/
// operator-reviewed per G-LOG-REVIEW, not an automated pass/fail gate --
// this writes a durable per-tick JSONL run log AND drives the live
// dashboard's world-state file so G-LIVE-VIEW is servable in parallel
// (start tools/ant-hive-world/dashboard.js against the same --sandbox-root
// separately; not spawned from here so the operator can stop/restart the
// dashboard independently of the run).
//
...
const AUTHORED_BY = 'claude-sonnet-5 (session a0264e43) -- untrained-network, no pretraining, early-minds generation';
