/**
 * handoff command module
 *
 * Creates developer handoff bundle by delegating to the legacy make-dev-handoff.js tool.
 */
...
Usage:
  node framework/runner/cli.js handoff --testcase <id> --runset <id> [options]

Options:
  --testcase <id>         Required. Testcase identifier
  --runset <id>           Required. Runset identifier
  --project-root <path>   Project root path
  --developer <name>      Optional. Developer name for bundle naming

Example:
  node framework/runner/cli.js handoff --testcase my_test --runset run_0001 --developer "Dev"
`.trim();

function findLegacyTool(projectRoot) {
  const toolName = 'make-dev-handoff.js';
  const searchPaths = [
...
export async function run(args) {
  ...
  if (!legacyPath) {
    console.error('Error: Legacy tool make-dev-handoff.js not found');
    console.error('Searched paths:');
    console.error('  - <framework>/playwright_phased_runner/runner/tools/make-dev-handoff.js');
    console.error('  - <cwd>/playwright_phased_runner/runner/tools/make-dev-handoff.js');
    process.exit(1);
  }
