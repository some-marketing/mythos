...
// Triad lanes (plan ant-sim-nine-mind-harness-triad-architecture, S2/S3).
// VERIFIER has no shape of its own to declare beyond a source version;
// SWEEPER's only shape fact is its ring-buffer width.
const { DEFAULT_WINDOW: SWEEPER_DEFAULT_WINDOW } = require('./sweeper-lane.js');
const VERIFIER_SOURCE_VERSION = 'v1';
...
function shapeDomain() {
  return {
    ...
    // Plan ant-sim-nine-mind-harness-triad-architecture, S2/S3: declaring
    // this unconditionally (not gated on whether any live hive actually has
    // a laneState this run) is deliberate -- the moment triad instrumentation
    // exists in the running engine at all, shape_hash moves for EVERY
    // generation, hive-B and the world mind included, forcing a fresh
    // lineage root rather than a silent partial-migration state. This is the
    // operator-authorized lineage spend (plan §3/§9), not an accident of
    // this field's placement.
    lanes: {
      verifier: { source_version: VERIFIER_SOURCE_VERSION },
      sweeper: { window_size: SWEEPER_DEFAULT_WINDOW }
    }
  };
}
...
// Plan ant-sim-nine-mind-harness-triad-architecture, S2/S3. VERIFIER has no
// state to serialize (zero trainable parameters, no cross-tick memory) --
// only SWEEPER's ring buffer round-trips. `laneState` here is the per-hive
// `{ sweeperState }` shape trainTick() consumes (train-tick.js). Absent
// sweeperState (VERIFIER-only or triad-off) serializes to `null`, and a
// restored generation with no `lanes` entry for a hive deserializes back to
// `null` -- SWEEPER starts with an empty buffer for that hive, same as a
// fresh run, rather than silently fabricating history that never happened.
function serializeLaneState(laneState) {
  if (!laneState || !laneState.sweeperState) return null;
  const { windowSize, buffer } = laneState.sweeperState;
  return { sweeper: { window_size: windowSize, buffer: buffer.map((entry) => ({ verb: entry.verb, reward: entry.reward })) } };
}

function deserializeLaneState(payload) {
  if (!payload || !payload.sweeper) return null;
  return {
    sweeperState: {
      windowSize: payload.sweeper.window_size,
      buffer: payload.sweeper.buffer.map((entry) => ({ verb: entry.verb, reward: entry.reward }))
    }
  };
}
...
function commitGeneration(checkpointRoot, state) {
  ...
  const mind = {
    hives: Object.fromEntries(hiveIds.map((hid) => [hid, {
      ...
      // Plan ant-sim-nine-mind-harness-triad-architecture, S2/S3: `null` for
      // every hive not opted into the triad this run -- state.laneStates is
      // optional and per-hive, same discipline as state.controllers.
      lanes: state.laneStates ? serializeLaneState(state.laneStates[hid]) : null
    }])),
...
function loadGeneration(checkpointRoot, id, opts = {}) {
  ...
  for (const [hid, entry] of Object.entries(mind.hives)) {
    ...
    // Plan ant-sim-nine-mind-harness-triad-architecture, S2/S3. Absent on
    // every generation committed before this field existed (including the
    // gen-150/300/450 lineage) -- deserializeLaneState(undefined) returns
    // null, and the caller starts that hive's SWEEPER with a fresh empty
    // buffer rather than fabricating history that was never recorded.
    laneStates[hid] = deserializeLaneState(entry.lanes);
  }
