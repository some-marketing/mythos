...
// environment by plan ant-hive-world-orwell-sim-containment, S4.
//
// Trivial round-trip: invokes bridge_step.py on Orwell, now INSIDE the
// S3-adversarially-verified container (image ant-hive-embodiment-bridge:s2,
// digest sha256:59ea5d22103b0906c1d8233f481b338a039b8c8f7dbdd7ef50e6b05d412d9802),
// using the exact flag set S3 verified against the full M1-M8 threat-model
// matrix (see _dev/reports/analysis/ant-hive-world-orwell-sim-containment-host-change-receipt.md,
// S3 section, "Exact `docker run` invocation verified"). Previously this ran
// bridge_step.py directly on Orwell's bare host via `venv\Scripts\python.exe`;
...
// S3-verified image identity (host-change-receipt.md, S2 + S3 sections).
const IMAGE = 'ant-hive-embodiment-bridge:s2';
const IMAGE_DIGEST = 'sha256:59ea5d22103b0906c1d8233f481b338a039b8c8f7dbdd7ef50e6b05d412d9802';
...
// -----------------------------------------------------------------------
const HOST = HOST_OVERRIDE || 'orwell';

function sshArgs() {
  return ['-o', 'ConnectTimeout=15', '-o', 'BatchMode=yes', HOST];
}

function assertImageDigestOrwell() {
  const result = spawnSync('ssh', [
    ...sshArgs(),
    `docker --context default inspect --format "{{.Id}}" ${IMAGE}`
  ], { encoding: 'utf8', timeout: 30000 });
  if (result.error || result.status !== 0) {
    throw new Error(`Could not verify ${IMAGE} digest on Orwell before use: ${result.error ? result.error.message : result.stderr}`);
  }
  const actual = result.stdout.trim();
  if (actual !== IMAGE_DIGEST) {
    throw new Error(`Refusing to run: ${IMAGE} on Orwell has digest ${actual}, expected the S3-verified ${IMAGE_DIGEST}. This is not the adversarially-evidenced image.`);
  }
}

function stepOnOrwell(steps, opts) {
  const skipDigestCheck = opts && opts.skipDigestCheck === true;
  if (!skipDigestCheck) assertImageDigestOrwell();
  const dockerCmd = [
