...
// ant-hive-world-embodiment-containment, S3.
//
// Adversarially evidences the S0 design memo's 8-row threat-model /
// acceptance matrix (M1-M8) against the S2-built image
// (ant-hive-embodiment-bridge:s2) actually running on the remote host, per each
...
//
// This script runs from the Mac and drives the remote host entirely over plain SSH
...
const SSH_ALIAS = process.env.ANT_HIVE_EMBODIMENT_HOST || 'ant-hive-embodiment-host';
...
const EXPECTED_IMAGE_ID = process.env.ANT_HIVE_EMBODIMENT_IMAGE_ID || '';
const BASE_IMAGE_DIGEST =
  'python:3.11-slim@sha256:db3ff2e1800a8581e2c48a27c3995339d47bdf046da21c7627accd3d51053a93';

const REMOTE_STAGE_DIR = process.env.ANT_HIVE_EMBODIMENT_STAGE_DIR ||
  'C:\\Users\\ant-hive-embodiment\\verify-containment-s3';
// No REMOTE_SCRATCH_DIR anymore: /scratch is a size-capped in-container
// tmpfs (repair 2026-07-18), not a Windows host directory.
const REMOTE_NEGTEST_DIR = process.env.ANT_HIVE_EMBODIMENT_NEGTEST_DIR ||
  'C:\\Users\\ant-hive-embodiment\\verify-containment-s3-negtest';
...
// -----------------------------------------------------------------------
// Repair 2026-07-18 (amendment operator repair 2026-07-18,
...
// -----------------------------------------------------------------------
// Probe script payloads (staged to the remote host, bind-mounted read-only into
...
// -----------------------------------------------------------------------
// Setup: stage probe scripts on the remote host
// -----------------------------------------------------------------------
...
const RESIDUAL_RISKS = [
  "Shared WSL2 kernel: all Linux containers on the remote host share ONE WSL2 kernel. A kernel-level LPE/container-escape CVE could reach the WSL2 VM or other containers; namespace/cgroup isolation does not protect against this. Operator-accepted 2026-07-17, NOT eliminated by any probe above.",
...
function printTable() {
  ...
  lines.push('S3 CONTAINMENT VERIFICATION -- ant-hive-world-embodiment-containment');
  lines.push('='.repeat(100));
  lines.push('');
...
function main() {
  ...
  let probePath;
  try {
    probePath = step('staging probe scripts on the remote host', setupStaging);
    ...
    step('cleaning up staged files on the remote host', () => cleanupStaging(keepStaging));
  }
