...
#
# Runs UnrealEditor-Cmd.exe against AntSimV2.uproject, executes
...
# D:\UnrealProjects\AntSimV2. Touches nothing under D:\HyperV.
#
# AntWorldProjection is a PRESERVED BASELINE as of 2026-08-06 -- this script
# refuses to run against it, whatever -ProjectRoot is passed.
#
...
param(
    [string]$Import = '',
    [string]$ProjectRoot = 'D:\UnrealProjects\AntSimV2',
...
$PreservedBaselineProject = 'AntWorldProjection'
if ($ProjectRoot -like "*$PreservedBaselineProject*") {
    "ANTWORLD_HOST_BASELINE_GUARD=REFUSED: $ProjectRoot names $PreservedBaselineProject, a preserved baseline that must never be written to again. The live projection target is AntSimV2."
    exit 5
}

# The .uproject is named for the project directory, so -ProjectRoot is the
# single source of truth and the two can never disagree.
$editor = Join-Path $EngineRoot 'Engine\Binaries\Win64\UnrealEditor-Cmd.exe'
$uproject = Join-Path $ProjectRoot ((Split-Path -Leaf $ProjectRoot.TrimEnd('\', '/')) + '.uproject')
