#!/usr/bin/env node
'use strict';
// CLI: mythos-custody-grant <path> --to-session <session_id> [--reason "..."]
// Writes a one-use operator override grant for a custody gate.
// Grant file: _dev/state/custody-gate/grants/<sha256(path:session)>.json

const fs = require('fs');
const pathMod = require('path');
const crypto = require('crypto');

const REPO_ROOT = process.env.MYTHOS_PROJECT_DIR || process.cwd();
const GRANTS_DIR = pathMod.join(REPO_ROOT, '_dev', 'state', 'custody-gate', 'grants');
...
function toRepoRelative(rawPath, repoRoot) {
...
function writeGrant(repoRelPath, toSession, reason, grantsDir) {
  const dir = grantsDir || GRANTS_DIR;
  fs.mkdirSync(dir, { recursive: true });
  const hash = grantHash(repoRelPath, toSession);
  const grantFile = pathMod.join(dir, hash + '.json');
...
if (require.main === module) {
  ...
  if (!rawPath) {
    process.stderr.write('Usage: mythos-custody-grant <path> --to-session <session_id> [--reason "..."]\n');
    process.exit(1);
  }
  if (!toSession) {
