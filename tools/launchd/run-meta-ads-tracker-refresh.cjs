...
//      write a ready-for-review HandoffSignal/1.0.
...
const TRACKER = path.join(REPO_ROOT, 'clients', '{CLIENT_CODE}', 'projects', 'meta-creative-iteration', 'outputs', '06-live-tracker', 'index.json');
...
if (trackerChanged) {
  ...
  if (diffCheck.status === 0) {
    ...
    const cm = spawnSync('/opt/homebrew/bin/git', [
      '-C', REPO_ROOT,
      '-c', 'user.name=Mythos Tracker Refresh',
      '-c', 'user.email=get+tracker@example-agency.com',
      'commit',
      '-m', `chore({CLIENT_CODE}/meta-ads): weekly tracker refresh ${nowUtc}`,
      '--no-verify',
      '--', TRACKER
    ], { encoding: 'utf8' });
...
if (exit !== 0 || hardStops > 0 || nullInsightsDetected) {
  ...
  const signalPath = path.join(SIGNAL_DIR, `ready-for-review__${nowStamp}__meta-ads-tracker-refresh-{CLIENT_CODE}.json`);
  const signal = {
    schema: 'HandoffSignal/1.0',
    signal_type: 'ready-for-review',
    lifecycle_state: 'live',
    source: 'launchd:meta-ads-tracker-refresh-{CLIENT_CODE}',
    scope: 'meta-ads-tracker-refresh-{CLIENT_CODE}',
    timestamp: nowUtc,
    artifacts: [
      path.relative(REPO_ROOT, runOut),
      '_dev/state/meta-ads-tracker/runs.jsonl',
      'clients/{CLIENT_CODE}/projects/meta-creative-iteration/outputs/06-live-tracker/index.json'
    ],
    decision_context_artifacts: [
      'clients/{CLIENT_CODE}/projects/meta-creative-iteration/outputs/05a-preregistration/{CLIENT_CODE}-halifax-open-house.json',
      'clients/{CLIENT_CODE}/projects/meta-creative-iteration/outputs/05a-preregistration/{CLIENT_CODE}-bhm-may-2026.json',
      'clients/{CLIENT_CODE}/projects/meta-creative-iteration/outputs/05a-preregistration/hh-bhm-may-2026.json'
    ],
    validation: {
      ran: true,
    ...
    grounding_mode: 'none',
    signal_scope: 'meta-ads-tracker-refresh-{CLIENT_CODE}',
    run_outcome: {
...
process.stdout.write(`[run-meta-ads-tracker-refresh] ts=${nowUtc} exit=${exit} changed=${trackerChanged} hard_stops=${hardStops} commit=${commitOk}\n`);
process.exit(exit);
