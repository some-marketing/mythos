#!/usr/bin/env bash
# update-obs-perspective.sh — replace an OBS image source's live file on a
# remote Windows host with a photo from this Mac.
#
# Flow: newest Photos-library photo -> JPEG (sips) -> back up the current
# image (on the remote host + a timestamped copy on the Mac) -> scp up ->
# overwrite the live OBS image file -> verify. OBS image sources auto-reload
# on file change.
#
# Usage:
#   OBS_HOST=youruser@yourhost tools/obs/update-obs-perspective.sh   # use the NEWEST photo in the Mac Photos library
...
#
# Config (override via env): OBS_HOST (required, e.g. youruser@yourhost), OBS_IMAGE, PHOTOS_ORIG
set -uo pipefail

OBS_HOST="${OBS_HOST:-}"
OBS_IMAGE="${OBS_IMAGE:-H:\\My Drive\\myperspective.jpeg}"   # Windows (cmd) path on the remote host
...
[ -n "$OBS_HOST" ] || die "set OBS_HOST to your remote host, e.g. OBS_HOST=youruser@yourhost $0"
...
if [ "$RESTORE" = 1 ]; then
  say "[restore] copying previous image back over the live OBS image on $OBS_HOST ..."
  ssh "${SSHO[@]}" "$OBS_HOST" "copy /Y \"%USERPROFILE%\\myperspective.prev.jpeg\" \"$OBS_IMAGE\"" 2>&1 | clean
  ssh "${SSHO[@]}" "$OBS_HOST" "dir \"$OBS_IMAGE\"" 2>&1 | clean | grep -iE "myperspective|File\(s\)" || true
  exit 0
fi
...
say "[target] $OBS_HOST :: $OBS_IMAGE"

if [ "$DRYRUN" = 1 ]; then say "[dry-run] not pushing. (re-run without --dry-run to apply)"; exit 0; fi

# 3. Back up the current image: fixed-name on the remote host (for --restore) + timestamped on the Mac
ts="$(date +%Y%m%dT%H%M%S)"
ssh "${SSHO[@]}" "$OBS_HOST" "copy /Y \"$OBS_IMAGE\" \"%USERPROFILE%\\myperspective.prev.jpeg\"" 2>&1 | clean
scp "${SSHO[@]}" "$OBS_HOST:myperspective.prev.jpeg" "$WORK/myperspective.backup.$ts.jpeg" 2>&1 | clean
[ -f "$WORK/myperspective.backup.$ts.jpeg" ] && say "[backup] $WORK/myperspective.backup.$ts.jpeg" || say "[backup] (remote copy made; mac pull skipped)"
...
scp "${SSHO[@]}" "$JPG" "$OBS_HOST:obs-new.jpg" 2>&1 | clean
ssh "${SSHO[@]}" "$OBS_HOST" "copy /Y \"%USERPROFILE%\\obs-new.jpg\" \"$OBS_IMAGE\"" 2>&1 | clean
