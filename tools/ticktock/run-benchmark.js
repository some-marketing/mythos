...
// T4 (sim-foundation-repairs): a tripped repeated-re-baseline detector must
// FILE a finding, not merely return a boolean. The finding is a real artifact
// written to the signals surface named in SKILL.md:
//   _dev/reports/signals/ticktock-rebaseline-frequency__<charter_id>__<cycle>.json
// (schema TickTockRebaselineFinding/1.0). DEFAULT_SIGNALS_DIR is that surface;
// a caller may override it per detection (the cycle-driver benchmark subcommand
// passes the signals dir through from its own argument).
const DEFAULT_SIGNALS_DIR = path.join(REPO_ROOT, '_dev', 'reports', 'signals');
const REBASELINE_FINDING_SCHEMA = 'TickTockRebaselineFinding/1.0';
...
// `options.lineage` supplies the re-baselining record. Lineage is an
// append-only ARRAY of re-baseline entries: both consumers walk entries
// (checkRebaselineFrequency's window and verifyLineageChain's link check), and
// a single object can never trip a 2-in-M window — the shape mismatch that
// left TT-003 counting nothing (C-F4b). Initial baseline = no entries ([]).
// A re-baseline APPENDS its entry to the prior file's chain
// (options.priorLineage, threaded through by record()); anything beyond the
// initial baseline has to be stated by whoever re-records, because a
// re-baseline whose reason is generated rather than supplied is a re-baseline
// nobody accounted for. The CLI exposes it as --prior-fingerprint / --reason /
// --review-artifact / --ratification / --triggering-cycle.
function computeFingerprint(spec, run, options = {}) {
  const { sandboxRoot, engineRoot } = run;
  const priorLineage = Array.isArray(options.priorLineage) ? options.priorLineage : [];
  const lineage = options.lineage
    ? [...priorLineage, { ...options.lineage }]
    : [...priorLineage];
  ...
  // Stamp the appended entry with the fingerprint it PRODUCED, so the next
  // re-baseline's prior_fingerprint_hash has a link to verify against. Lineage
  // is provenance, outside the identity hash, so stamping after the hash is
  // well-ordered and cannot perturb it.
  if (options.lineage) {
    lineage[lineage.length - 1].new_fingerprint_hash = fingerprint.fingerprint_hash;
  }
  return fingerprint;
}
...
function checkRebaselineFrequency(lineageEntries, options = {}) {
  ...
  // T4 (sim-foundation-repairs): a tripped detector FILES a finding, it does
  // not just self-attest. The artifact is written to the signals surface at
  //   <signals_dir>/ticktock-rebaseline-frequency__<charter_id>__<cycle>.json
  // with schema REBASELINE_FINDING_SCHEMA, deduplicated on the pair
  // (charter_hash, cycle_index): a second detection of the same pair never
  // double-writes -- the path derives from charter_id (unique per charter, so
  // 1:1 with charter_hash) plus cycle_index, and an existing file at that path
  // IS that pair already recorded. Absence of charter identity (a direct call
  // from the unit test) is tolerated: no artifact surface is named, so nothing
  // is written, and the detector's verdict is unchanged.
  let finding_artifact = null;
  if (exceeded && options.charter_id !== undefined && options.charter_hash) {
    const cycleIndex = currentCycle === undefined ? highest : currentCycle;
    const signalsDir = options.signals_dir || DEFAULT_SIGNALS_DIR;
    finding_artifact = path.join(signalsDir, `ticktock-rebaseline-frequency__${options.charter_id}__${cycleIndex}.json`);
    if (!fs.existsSync(finding_artifact)) {
      const finding = `re-baselining occurred in ${count} of the last ${m} cycles, above the charter threshold of ${n} -- the frame is too tight or the loop is drifting deliberately; an explicit operator decision is required before further cycles run`;
      const artifact = {
        schema: REBASELINE_FINDING_SCHEMA,
        charter_hash: options.charter_hash,
      ...
      fs.writeFileSync(finding_artifact, JSON.stringify(artifact, null, 2) + '\n');
    }
  }
...
function record(options = {}) {
  ...
  // On a re-baseline, carry the existing chain forward: the prior file's
  // lineage array is what the new entry appends to. An unreadable or
  // non-array prior lineage starts the chain at this entry rather than
  // inventing history.
  if (options.lineage && options.priorLineage === undefined) {
    const fpPath = path.resolve(options.fingerprintPath || DEFAULT_FINGERPRINT_PATH);
    if (fs.existsSync(fpPath)) {
      try {
        const prior = JSON.parse(fs.readFileSync(fpPath, 'utf8'));
        if (Array.isArray(prior.lineage)) options.priorLineage = prior.lineage;
      } catch (e) { /* chain starts at this entry */ }
    }
  }
