...
function buildRun(journalPath) {
  ...
  for (const [i, phase] of ['tt.orient', 'tt.observe'].entries()) {
    ...
    journal.completePhase(journalPath, { charter_hash: CHARTER_HASH, cycle_index: 0, phase_id: phase }, [artifact]);
  }
  return artifacts;
}
...
// ---------------------------------------------------------------------------
{
  ...
  journal.completePhase(j, { charter_hash: CHARTER_HASH, cycle_index: 0, phase_id: 'tt.research' }, [artifact]);
...
// ---------------------------------------------------------------------------
{
  ...
  // 4d. A genuinely fresh start is still a fresh start.
  const n = freshJournal();
  const empty = journal.resolveResume(n);
  check('an empty journal with no anchor still resumes as a fresh start', empty.resumable === true && empty.fresh_start === true, empty);
}
...
// would either crash or silently pass.
{
  ...
  journal.completePhase(k, { charter_hash: CHARTER_HASH, cycle_index: 0, phase_id: 'tt.orient' }, [artifact]);
  fs.writeFileSync(k, '');
